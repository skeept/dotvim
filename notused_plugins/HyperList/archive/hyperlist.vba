...
doc/hyperlist.txt	[[[1
1331
*hyperlist.txt*   The VIM plugin for HyperList (version 2.1)
...
This plugin incorporates encryption. You can encrypt any part of a HyperList or
take advantage of the autoencryption feature by making the HyperList a dot file
- i.e. prefixing the file name wiht a dot (such as ".password.hl"). You can use
...
suffix ".woim" for backward compatability so that these file typeas are also
...
When working wiht HyperLists in VIM:
...
include WOIM lists in files with a ".txt", and the following example would
...
HyperList. My colleague Egil Möller's personal website: http://redhog.org/
helped in the early refinements.
...
as outlined here:
	...
	4.3 Content
		...
		4.3.2 Additive
			...
			4.3.2.2 Comment  
			4.3.2.3 Quote
			4.3.2.4 Change Markup
	4.4 Separator

Look familiar? Yes, it is part of the table of contents at the beginning of
this documentation -- even that is a valid HyperList. If you are reading this
document as a PDF file, you may use the table of contents to jump directly to
any part of the document, as it consists of clickable links. 
...
The hash sign ("#") near the end will be discussed later in this article.
...
An Element is either an "Operator", a "Qualifier", a "Tag" or a "Description".
...
------------------------------------------------------------------------------
4.3.1.2 Tags                                                  *HyperList-Tags*

A Tag is any attribute describing the Content. It ends in a colon and a space.

Examples of Tags could be: "Location = Someplace:", "Color = Green:", "Strength
= Medium:" and "In Norway:". Anything that tags or gives additional information
or description to the Content.
...
A hash sign ("#") followed by the name of an Item, list or anything else is a
Reference. An example would be a Reference to a website, such as
#http://www.isene.com/, or a file, #file:/path/to/filename. A Reference with
spaces must be enclosed in single quotation marks: #'This is a Reference', as
...
You may use a unique concatenation of a path to shorten it, such as #'Somewhere
higher up/One lev.../Ref...'. The three periods indicate concatenation.
...
this need, Change Markup is introduced.

If "<<" is added at the end of an Item, it is slated for deletion. To show that
an Item should be moved, add ">>" at the end followed by a Reference showing to
which Item it should be moved below.
...
HyperList, prefix the Item with "<<>>". Inside the angle brackets you may
...
First, here is a simple list showing what HyperList is all about:
	...
	HyperList Item parts (in sequence): 
		...
		Content (can be an Element and/or an Additive)
			Element
				Either an Operator, Qualifier, Tag, or Description
			Additive    
				Either a Reference, Comment, Quote, or Change Markup
		Separator
	...
	HyperList
		[1+] HyperList Item
			...
			Content; AND/OR: 
				...
				Additive; AND/OR: 
					Reference
						+ A "#" followed by an Item name or Identifier, list name or 
								...
								An Item consisting only of a Reference is a redirection
									For a transition Item = Jump to referenced Item and execute
										+ If the redirect is to jump back after executing 
										 ...
										 hash at the beginning (##'Referenced Item')
					...
					Change Markup; OR: 
						...
						Changed Item
							Prefix an Item with "<<>>" to signify that a change has been made
...
HyperList definition itself; Geir Isene <g@isene.com>. More at http//isene.com 
