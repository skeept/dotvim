...
"	DechoWF "(ManPageView) special vim handler"
	call manpageview#ManPageVim(topic)
...
"	DechoWF "(ManPageView) special tex handler"
    let ext= "tex"
    call manpageview#ManPageTexLookup(0,topic)
"    call Dret("manpageview#ManPageView")
    return
   endif
  ...
  elseif ext == "vim"
"   DechoWF "(ManPageView) special vim handler"
   call manpageview#ManPageVim(substitute(topic,'\.vim$','',''))
"   call Dret("manpageview#ManPageView")
   return

  elseif ext == "tex"
   call manpageview#ManPageTexLookup(0,substitute(topic,'\.tex$',"",""))
"   call Dret("manpageview#ManPageView")
   return
  endif
...
"    DechoWF "found pre-exiting manpageview window, re-using it"
...
"   DechoWF "(ManPageView) not-multimanpage: mapping PageUp/Down to do ctrl-f, ctrl-b"
  ...
  nnoremap <buffer> <c-]>       :call manpageview#ManPageView(v:count1,expand("<cWORD>"))<cr>
...
"   DechoWF "(ManPageView) restoring settings"
   call s:MPVRestoreSettings()
"   call Dret("manpageview#ManPageView : no manpage exists for <".topic.">")
   return
...
"	call Decho("(prev) history[".s:ihist."]=".string(s:history[s:ihist]))
  ...
  nno <buffer> <s-down>	:call manpageview#History(-v:count1)<cr>
  nno <buffer> <s-up>	:call manpageview#History( v:count1)<cr>
